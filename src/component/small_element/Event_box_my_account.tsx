...
import {UserBookedEventsInfoMyAccount} from "../../data/dataForMyAccount";
import { Calendar, Clock, MapPin, Tag } from "lucide-react";
import { serverUrl } from '@/src/data/severUrl';

//type EventBoxProps = { eventInfo: UserBookedEventsInfoMyAccount };
let eventInfo:UserBookedEventsInfoMyAccount[number];

const Event_box_my_account = ({eventInfo}:{eventInfo:UserBookedEventsInfoMyAccount[number]} ) => { 
    ...
    <div className=" w-[1145px] h-[150px] bg-white rounded-[10px] m-[10px] p-[10px] box-border flex border-[1px] border-gray-300 hover:shadow-lg cursor-pointer">
      ...
      <div className=" ml-[15px] flex flex-col justify-between">
        <div>
          ...
          <div className=" text-[14px] text-gray-500 mt-[3px]">Tickets Booked: {eventInfo.number_of_tickets_booked}</div>
        </div>
      </div>
    </div>
  </div>;
...
export default Event_box_my_account;